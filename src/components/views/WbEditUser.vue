...
//        console.log(this)
      ...
      onSubmit () {
        ...
        for (i = 0; i < this.newList.length; i++) {
          tempdevices = tempdevices + ',' + this.newList[i]
        }
        var params = {
          'operator_id': userid,
        ...
        .then(response => {
          ...
          api.request('get', 'user/view?operator_id=' + userid + '&user_id=' + currentUserId)
            .then(response => {
              ...
              console.log('msg.priority is', msg.priority)
              this.form.priority = msg.priority
              this.form.gender = msg.gender
              this.form.pass = msg.pass_word
              this.form.phone = msg.mobile
              this.radio = 0
              this.gender = msg.gender
              this.form.role = msg.role
              this.chooseList = msg.related_devices
              // 设置组织机构默认值
              console.log('size is', this.options.length)
              var i, j
              for (i = 0; i < this.options.length; i++) {
                console.log('valueis', msg.agency_id, this.options[i].id)
                if (Number(msg.agency_id) === Number(this.options[i].id)) {
                  j = i
                  break
                }
              }
              console.log('j is', j)
              this.value = this.options[j] // msg.agency_name
