      ...
      <div class="row">
        <div class="col-md-12">
          <div style="margin-top: 15px;">
            <el-input placeholder="请输入用户名关键字" v-model="input5" class="input-with-select">
          ...
          <waterfall :line-gap="250" :watch="arrayData" style="margin-top: 10px">
            ...
            >
              <el-card :body-style="{ padding: '10px' }">
                <img v-bind:src="item.avatar" class="rimage" style="text-align: center" >
                <div style="padding: 14px;">
                  <div style="color: rgba(0,172,214,0.93);font-size: 16px">{{item.nickname}}</div>
                  <div>性别:<span v-if="item.gender==='0'">女</span><span v-else-if="item.gender==='1'">男</span><span v-else>保密</span> </div>
                  <div>注册时间:{{item.registerat}}</div>
                  <div>登录时间:{{item.loginat}}</div>
                  <div>用户等级:{{item.level}}</div>
                  <div>动态数:{{item.level}}</div>
                  <div>话题数:{{item.level}}</div>
                  <div>评论数:{{item.level}}</div>
                  <div class="bottom clearfix">
                    <time class="time">{{ currentDate }}</time>
                    <el-button type="text" class="button">timeline</el-button>
                  </div>
                </div>
              </el-card>
              <!--
